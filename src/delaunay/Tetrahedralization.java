...
public class Tetrahedralization {
     ...
     */
    public List<Vec3[]> getVoronoiRegion(final Vertex v) {
        assert v != null && v.getAdjacent() != null;

        final ArrayList<Vec3[]> faces = new ArrayList<Vec3[]>();
        v.getAdjacent().visitStar(v, new StarVisitor() {
    ...
    public static List<Vec3> getSurroundingVertices(List<Vec3> points){
    	...
    	Collection<Vertex> vert = t.getNeighbors(center);
    	List<Vec3> neigh = new ArrayList<Vec3>(vert.size());
    	for (Vertex v : vert){
    		neigh.add(v.asVec3());
    	}
    	return neigh;
