...
public class L10_Structure extends FCCStructure{
	...
	private static Vec3[] perfectNeighbors = new Vec3[]{
	...
	static{
		float[] tol_l = new float[]{0f, 7f, 7f, 6f, 4.5f, 7f};
		float[] tol_u = new float[]{13f, 10f, 7f, 6f, 4.5f, 7f};
	...
	public int identifyAtomType(Atom atom, NearestNeighborBuilder<Atom> nnb) {
		...
		if (d_cont.size() < 10) return 7;
			...
			//type 1: A-A-B (one neighbor of each element)
			//type 2: A-A-A (bond consists of only one type)
			if (bonds[0][0] == 4 && bonds[0][2] == 2) return 0; //L10
			if (bonds[0][0] == 2 && bonds[0][1] == 3 && bonds[0][2] == 1) return 1; //APB
			if (bonds[0][0] == 2 && bonds[0][2] == 1 && bonds[1][2] == 2 && bonds[1][1] == 2 && bonds[1][0] == 2) return 2; //CSF
			if (bonds[0][0] == 2 && bonds[0][2] == 1 && bonds[1][0] == 2 && bonds[1][1] == 4) return 3; //SISF
	...
	public boolean isRBVToBeCalculated(Atom a) {
		return a.getType()!=getSurfaceType() && a.getType()>=4;
	}
}
