...
class RangeSliderUI extends BasicSliderUI {
    ...
    public void paintTrack(Graphics g) {
		...
		float[][] bar = scheme.getColorBar();
    	
    	if (slider.getOrientation() == JSlider.HORIZONTAL){
    		for (int i=0; i<scheme.getColorBar().length-1; i++){
    			int index = ColorTable.isColorBarSwapped() ? bar.length-1-i : i; 
    			int index2 = ColorTable.isColorBarSwapped() ? bar.length-2-i : i+1;
    			
    			GradientPaint gp = new GradientPaint(dim.width*(1f/bar.length)*i, 0,
    					new Color(bar[index][0], bar[index][1], bar[index][2]),
    					dim.width*(1f/bar.length)*(i+1), 0,
    					new Color(bar[index2][0], bar[index2][1], bar[index2][2]));
    			Rectangle2D rect2D = new Rectangle2D.Double(dim.width*(1f/bar.length)*i, 0, dim.width*(1f/bar.length)*(i+1), dim.height);
    		...
    		for (int i=0; i<scheme.getColorBar().length-1; i++){
    			...
    			GradientPaint gp = new GradientPaint(0, dim.height*(1f/bar.length)*i,
    					new Color(bar[index][0], bar[index][1], bar[index][2]),
    					0, dim.height*(1f/bar.length)*(i+1),
    					new Color(bar[index2][0], bar[index2][1], bar[index2][2]));
    			Rectangle2D rect2D = new Rectangle2D.Double(0, dim.height*(1f/bar.length)*i, dim.width, dim.height*(1f/bar.length)*(i+1));
    ...
    private void paintLowerThumb(Graphics g) {
        ...
        Shape thumbShape = createThumbShapeLower(w - 1, h - 1);
    ...
    private void paintUpperThumb(Graphics g) {
        ...
        // Create default thumb shape.
        Shape thumbShape = createThumbShapeUpper(w - 1, h - 1);
    ...
    private Shape createThumbShapeLower(int width, int height) {
        // Triangle
    ...
    private Shape createThumbShapeUpper(int width, int height) {
        // Triangle
    	Polygon shape = new Polygon();
    	if (slider.getOrientation() == JSlider.HORIZONTAL){
	    	shape.addPoint(0, 0);
	    	shape.addPoint(width/2, height);
