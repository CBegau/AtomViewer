...
public class LoadBalancingProcessingModule extends ClonableProcessingModule {
	...
	//The selected file to be opened
	...
	public ProcessingResult process(AtomData data) throws Exception {
		final LoadBalancingDataContainer dc = new LoadBalancingDataContainer();
		
		dc.processData(data, dataFile);
	...
	/**
	 * The load balancing data read from files are processed and stored in instances of this class
	 */
...
//			viewer.drawLegendThisFrame("0.8", "1.0",  "1.2");
		...
		private class LoadBalanceInfo implements Pickable{
	...
	/**
	 * The graphical interface for visualization options
	 *
	 */
	public static class JLoadBalancingControlPanel extends JDataPanel {
		...
		public JLoadBalancingControlPanel() {
			...
			wireFrameCheckbox.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent arg0) {
					drawAsWireframe = wireFrameCheckbox.isSelected();
					viewer.reDraw();
				}
			});
