...
public final class VacancyDataContainer2 extends ParticleDataContainer<Vacancy>{
	...
	public void findVacancies(final AtomData data, final float nnd_tolerance){
		...
		final int defaultNumNeigh = cs.getNumberOfNearestNeighbors();
		...
		for (int k = 0; k < nextToVancanyCandidateAtoms.size(); k++) {
			...
			Tetrahedralization t = new Tetrahedralization();			
			Vertex center = new Vertex(0., 0., 0.);
			t.insert(center);
			for (Vec3 p : nnb) {
				t.insert(new Vertex(p.x, p.y, p.z));
			}
			
			//get the voronoi cell
			List<Vec3[]> voronoi;
			try {
				voronoi = t.getVoronoiRegion(center);
			} catch (Exception e){
				continue;
			}
			...
			List<Vec3> uniqueVertices = new ArrayList<Vec3>();
			for (Vec3[] av : voronoi){
				for (Vec3 v : av){
					boolean exists = false;
					for (Vec3 v2 : uniqueVertices)
						if (v2.getSqrDistTo(v)<1e-6) {
							exists = true;
							break;
						}
			...
			for (Vec3 point : uniqueVertices){
				//Exclude points that are very close to an 
				if (point.getLengthSqr() < 0.25f*minDistanceToAtom)
					continue;
				if (point.getLengthSqr()>1e4) continue;
				
				
				Vertex pointVertex = new Vertex(point.x, point.y, point.z);
				t.insert(pointVertex);
				
				Collection<Vertex> neighhull = t.getNeighbors(pointVertex);
				if (neighhull.size() < 4) continue;
				
				List<Vec3> hull = new ArrayList<Vec3>();
				for (Vertex v : neighhull){
					Vec3 vv = v.asVec3();
					if (vv.getLengthSqr()<1e4)
						hull.add(vv);
				}
...
//				t.delete(pointVertex);
				t = new Tetrahedralization();
				t.insert(center);
				for (Vec3 p : nnb) {
					t.insert(new Vertex(p.x, p.y, p.z));
				}
		...
		for(int i = 0; i < possibleVacancyList.size(); i++) {
			...
			Vacancy v = new Vacancy(possibleVacancyList.get(i));
	...
	public Vec3 getChebyshevCenter(List<Vec3> list) {
