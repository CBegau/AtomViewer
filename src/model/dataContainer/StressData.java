...
public class StressData extends DataContainer {
	private ArrayList<StressValue> stresses = new ArrayList<StressValue>();

	private static float[] globalMinStress = new float[7];
	private static float[] globalMaxStress = new float[7];
	private static int showStressValue = 0;
	private static float lowerLimit = 0f;
	private static float upperLimit = 1f;
	private static boolean invertInterval = false;
	private static JStressControlPanel dataPanel;
	
	private static float minimalVonMisesStress = 0f;
	...
	static void setLowerLimit(float lowerLimit) {
		...
		if (lowerLimit>StressData.upperLimit) StressData.lowerLimit = StressData.upperLimit;
		else StressData.lowerLimit = lowerLimit;
	}
	
	static void setUpperLimit(float upperLimit) {
		if (upperLimit>1f) StressData.upperLimit = 1f;
		if (upperLimit<StressData.lowerLimit) StressData.upperLimit = StressData.lowerLimit;
		else StressData.upperLimit = upperLimit;
	}
	...
	public void drawSolidObjects(ViewerGLJPanel viewer, GL3 gl, RenderRange renderRange, boolean picking, BoxParameter box) {
		...
		float max = globalMaxStress[showStressValue];
		
		float lowLimit = lowerLimit*(max-min) + min; 
		float upLimit = upperLimit*(max-min) + min;
		...
		for (int i = 0; i < stresses.size(); i++) {
			...
			if (renderRange.isInInterval(c)){
				if ( (!invertInterval && c.getStress(showStressValue) >= lowLimit && c.getStress(showStressValue) <= upLimit) ||
						(invertInterval && (c.getStress(showStressValue) <= lowLimit || c.getStress(showStressValue) >= upLimit))){
					objects.add(c);
				}
			}
		}
	...
	public static class JStressControlPanel extends JDataPanel {
		...
		public JStressControlPanel() {
			...
			stressComboBox.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					StressData.showStressValue = stressComboBox.getSelectedIndex();
			...
			rangeSlider.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent e) {
					StressData.setLowerLimit(rangeSlider.getValue()*0.01f);
					StressData.setUpperLimit(rangeSlider.getUpperValue()*0.01f);
		...
		public void resetValues(){
			this.rangeSlider.setValue((int)(lowerLimit*100));
			this.rangeSlider.setUpperValue((int)(upperLimit*100));
			this.stressComboBox.setSelectedIndex(showStressValue);
			this.invertIntervalCheckBox.setSelected(invertInterval);
		}
	...
	public boolean processData(AtomData data) throws IOException {
		...
		try {
			...
			if (format.equals("A")) { // Ascii-Format
				...
				while (s != null) {
					...
					if (stress[6] > minimalVonMisesStress) {
		...
		for (int i = 0; i < 7; i++) {
			if (minStress[i] < globalMinStress[i]) globalMinStress[i] = minStress[i];
			if (maxStress[i] > globalMaxStress[i]) globalMaxStress[i] = maxStress[i];
		}
		return true;
	}
